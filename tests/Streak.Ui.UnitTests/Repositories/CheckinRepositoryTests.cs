namespace Streak.Ui.UnitTests.Repositories;
...
public class CheckinRepositoryTests
{
    ...
    [Fact]
    public async Task GetKeysByHabitIdsAsync_ShouldReturnEmpty_WhenHabitIdsCollectionIsEmpty()
    {
        await using var context = TestDbContextFactory.CreateContext(out var connection);
        await using (connection)
        {
            ...
            var result = await sut.GetKeysByHabitIdsAsync([]);
    ...
    [Fact]
    public async Task GetKeysByHabitIdsAsync_ShouldProjectKeysWithoutTrackingCheckinEntities()
    {
        await using var context = TestDbContextFactory.CreateContext(out var connection);
        await using (connection)
        {
            ...
            context.Checkins.AddRange(
                new Checkin
                {
                    ...
                    Notes = "Older note",
                    ProofImageUri = "Habit-1/2025/01/2025-01-01/proof.jpg"
                },
                new Checkin
                {
                    HabitId = 1,
                    ...
                    Notes = "Newer note",
                    ProofImageUri = "Habit-1/2025/01/2025-01-03/proof.jpg"
                },
                new Checkin
                {
                    HabitId = 2,
                    ...
                    Notes = "Run note",
                    ProofImageUri = "Habit-2/2025/01/2025-01-02/proof.jpg"
                },
                new Checkin
                {
                    HabitId = 3,
                    CheckinDate = "2025-01-03",
                    Notes = "Excluded note",
                    ProofImageUri = "Habit-3/2025/01/2025-01-03/proof.jpg"
                });
            await context.SaveChangesAsync();
            context.ChangeTracker.Clear();
            ...
            var result = await sut.GetKeysByHabitIdsAsync([2, 1, 2], "2025-01-02", "2025-01-03");

            result.Select(x => $"{x.HabitId}:{x.CheckinDate}")
                .Should()
                .Equal("1:2025-01-03", "2:2025-01-02");
            context.ChangeTracker.Entries<Checkin>().Should().BeEmpty();
        }
    }
    ...
    [Fact]
    public async Task GetKeysByHabitIdsAsync_ShouldThrowArgumentOutOfRangeException_WhenAnyHabitIdIsNotPositive()
    {
        var dbContextMock = new Mock<StreakDbContext>(new DbContextOptionsBuilder<StreakDbContext>().Options);
        var sut = new CheckinRepository(dbContextMock.Object);

        var act = () => sut.GetKeysByHabitIdsAsync([0, 1]);
