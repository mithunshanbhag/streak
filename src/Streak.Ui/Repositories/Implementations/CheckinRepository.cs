...
public class CheckinRepository(StreakDbContext dbContext) : SqlGenericRepositoryBase<Checkin, CheckinKey>(dbContext), ICheckinRepository
{
    ...
    public async Task<IReadOnlyList<Checkin>> GetByHabitIdsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var normalizedHabitIds = NormalizeHabitIds(habitIds);
        if (normalizedHabitIds.Length == 0) return [];
    ...
    public async Task<IReadOnlyList<CheckinKey>> GetKeysByHabitIdsAsync(
        IReadOnlyCollection<int> habitIds,
        string? fromDate = null,
        string? toDate = null,
        CancellationToken cancellationToken = default)
    {
        ...
        return await ApplyDateRange(
            ...
            .OrderBy(x => x.HabitId)
            .ThenByDescending(x => x.CheckinDate)
            .Select(x => new CheckinKey(x.HabitId, x.CheckinDate))
            .ToListAsync(cancellationToken);
    }
    ...
    private static int[] NormalizeHabitIds(IReadOnlyCollection<int> habitIds)
    {
