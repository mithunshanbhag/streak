namespace Streak.Ui.Constants;

public static class StreakExportStorageConstants
{
    public const string ExportWorkingDirectoryName = "ExportWorking";

    public const string AndroidRootDirectoryName = "Streak";

    public const string BackupsDirectoryName = "Backups";

    public const string ManualBackupsDirectoryName = "Manual";

    public const string AutomatedBackupsDirectoryName = "Automated";

    public const string ManualBackupsDisplayDirectoryPath = "Downloads/Streak/Backups/Manual";

    public const string AutomatedBackupsDisplayDirectoryPath = "Downloads/Streak/Backups/Automated";
}
